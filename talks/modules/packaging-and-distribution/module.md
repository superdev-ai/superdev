<!-- superdev:generated source=MOD-0011 revision=4091 hash=9d4d58cb70c6208d66dbe3e09cd4599ee77b9381ec25fccbb183a5edf40cdc24 -->
...
|---|---|---|
| Empty States | with no sync_peers rows at all (the normal state today), peers prints as 0 rather than erroring or omitting the field.; A project whose migrations define no enumerated state column produces no findings | Show cloud sync status, Synchronize with the cloud, Preview a sync, Refuse a state column nothing can move off its opening value |
...
| State Machine Violations | running the command repeatedly, or with any combination of flags, always yields the identical refusal and exit code; there is no connected state to transition into so no invalid transition can occur.; sync cannot be called mid-transfer or interrupted, because no transfer ever starts; every call is a fresh, complete no-op refusal.; A column enumerated with a single allowed value is a constant, not a state, and is skipped | Connect cloud sync, Synchronize with the cloud, Resolve sync conflicts, Refuse a state column nothing can move off its opening value |
...
| Data Migration States | A table rebuilt by a later migration is read from its last definition, and one dropped is not read at all | Refuse a state column nothing can move off its opening value |
