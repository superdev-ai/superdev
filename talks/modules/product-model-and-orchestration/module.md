...
| Duplication | A second goal, milestone or module with the same name is refused, naming the one that exists; A statement already recorded is refused, naming the record and the direction it was recorded as; A second record with the same name in the same module is refused, naming the one that has it | Accept a feature, Author the product map after initialization, Record what the product deliberately does not do, Author the rest of the product map |
...
| Deletion Semantics | Retiring keeps the record and its history, and stops it counting toward progress | Author the product map after initialization |
...
| 20 | Compliance and product tests | Filled | 124 assertions, eight validators and twenty-six release conditions, all re-run before a release. |
